    ...
    export default function LecturerItemRequest() {
        ...
        const [sectionBPage, setSectionBPage] = useState<number[]>([]);
        ...
        const handlePageChange = (selectedItem: { selected: number }) => {
            ...
            setSectionBPage(prev => {
                const updated = [...prev];
                updated[selectedItem.selected] = 0;
                return updated;
            });
        };

        const fullData = fileData[currentFileIndex]?.data || [];
        const currentSectionBPage = sectionBPage[currentFileIndex] || 0;

        const currentData = fullData.slice(
            currentSectionBPage * ITEMS_PER_PAGE,
            (currentSectionBPage + 1) * ITEMS_PER_PAGE
        );
    ...
    // Helper function to calculate time until next Monday 12 AM
        ...
        const handleExcelDataChange = (sectionA: any[], sectionAHeaders: string[], header: string[], data: any[], extractedValues: any[], downloadURL: string) => {
            ...
            setSectionBPage(prev => [...prev, 0]); // New page starts at 0
                                        ...
                                        <div className="mt-8">
                                            ...
                                            <div className="mt-4 flex flex-col items-center justify-between">
                                                <div className="text-sm text-gray-500 mb-2">
                                                    Showing {currentSectionBPage * ITEMS_PER_PAGE + 1} – {Math.min((currentSectionBPage + 1) * ITEMS_PER_PAGE, fullData.length)} of {fullData.length} items
                                                </div>
                                                <ReactPaginate
                                                    previousLabel={<HiChevronLeft className="w-4 h-4" />}
                                                    nextLabel={<HiChevronRight className="w-4 h-4" />}
                                                    breakLabel={'...'}
                                                    pageCount={Math.ceil(fullData.length / ITEMS_PER_PAGE)}
                                                    forcePage={currentSectionBPage}
                                                    onPageChange={({ selected }) => {
                                                        setSectionBPage((prev) => {
                                                            const updated = [...prev];
                                                            updated[currentFileIndex] = selected;
                                                            return updated;
                                                        });
                                                    }}
